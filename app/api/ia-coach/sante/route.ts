...
export async function POST(request: Request) {
  ...
  const validTags = (tags ?? []).filter((t): t is string => (ANTECEDENTS_OPTIONS as readonly string[]).includes(t));
  ...
  const { error } = await admin
    .from("ia_coaching")
    .update({ antecedents_tags: validTags, antecedents_details: cleanDetails })
